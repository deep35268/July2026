...
# ============ HIGH PRIORITY ORIGINAL HD LANDSCAPE POSTER ENGINE ============

async def fetch_cinemeta_ai_poster(query: str, is_series: bool = False) -> Optional[str]:
    """ ਬੈਕਅੱਪ ਵਜੋਂ Cinemeta ਤੋਂ HD Landscape Backdrop ਪ੍ਰਾਪਤ ਕਰਨਾ """
    try:
...
async def get_landscape_poster_only(movie_name: str, is_series: bool = False) -> Optional[str]:
    """ 
    ਸਭ ਤੋਂ ਪਹਿਲੀ ਪਹਿਲ: 100% Original HD Landscape Poster (Logo/Movie Name Included)
    """
    # 1st Priority: TMDB Backdrop HD Original Link
    try:
        ...
        logger.error(f"TMDB backdrop fetching error: {e}")

    # 2nd Priority: Cinemeta HD Landscape Poster
...
async def _process_with_lock(bot, filename, caption, media_info, base_name):
    ...
    try:
        ...
        # 🎯 FETCH ORIGINAL HD LANDSCAPE POSTER ONLY
        final_poster = await get_landscape_poster_only(base_name, is_series)

        if not final_poster:
            logger.info(f"❌ Original HD Landscape Poster NOT found for '{base_name}'. Skipping post creation.")
            return
    ...
    try:
        ...
        if is_update and movie_doc.get("message_id"):
            ...
            if sent_msg:
                ...
                logger.warning(f"Update failed for {base_name}, avoiding duplicates.")
                return None

        # --- NEW POST CASE (Direct Original HD Landscape Poster Send) ---
        try:
